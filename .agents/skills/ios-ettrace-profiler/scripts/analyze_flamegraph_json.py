...
This helper intentionally accepts only the Homebrew ETTrace v1.1.x processed
...
ETTrace v1.1.x stores `duration` as inclusive time on every real frame and
...
def node_weight(node: dict[str, Any]) -> float:
    """Return the inclusive `duration` stored on one ETTrace v1.1.x node."""
...
def thread_root_node(payload: dict[str, Any]) -> dict[str, Any] | None:
    """Return the top-level `nodes` tree from ETTrace v1.1.x processed JSON."""
...
def parse_flamegraph(path: Path):
    ...
    if "threadNodes" in payload:
        raise ValueError(
            ...
            "Use Homebrew ETTrace v1.1.x output_<thread>.json with top-level `nodes`.",
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "json",
        type=Path,
        help="Path to ETTrace v1.1.x processed output_<thread>.json.",
    )
