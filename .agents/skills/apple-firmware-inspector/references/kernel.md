...
```bash
ipsw kernel version kernelcache.release.$LATEST_IPHONE_DEVICE
```

**Decompress kernelcache:**
```bash
ipsw kernel dec kernelcache.release.$LATEST_IPHONE_DEVICE --output kernelcache.decompressed
```

**List kernel extensions:**
```bash
ipsw kernel kexts kernelcache.release.$LATEST_IPHONE_DEVICE
```

**JSON output:**
```bash
ipsw kernel kexts --json kernelcache.release.$LATEST_IPHONE_DEVICE
```
...
```bash
ipsw kernel extract kernelcache.release.$LATEST_IPHONE_DEVICE sandbox
```

**Extract KEXT to specific directory:**
```bash
ipsw kernel extract kernelcache.release.$LATEST_IPHONE_DEVICE sandbox --output ./kexts/
```

**Extract all KEXTs:**
```bash
ipsw kernel extract kernelcache.release.$LATEST_IPHONE_DEVICE --all --output ./kexts/
```

**Extract with specific architecture:**
```bash
ipsw kernel extract kernelcache.release.$LATEST_IPHONE_DEVICE IOKit --arch arm64e
```
...
```bash
ipsw kernel kexts --diff "$OLDER_KERNELCACHE" "$NEWER_KERNELCACHE"
```
...
**Dump syscall table:**
```bash
ipsw kernel syscall kernelcache.release.$LATEST_IPHONE_DEVICE
```

**Dump mach_traps:**
```bash
ipsw kernel mach kernelcache.release.$LATEST_IPHONE_DEVICE
```

**Search for specific syscall:**
```bash
ipsw kernel syscall kernelcache.release.$LATEST_IPHONE_DEVICE | grep execve
```
...
**Dump MIG subsystems:**
```bash
ipsw kernel mig kernelcache.release.$LATEST_IPHONE_DEVICE
```
...
```bash
ipsw kernel symbolicate kernelcache.release.$LATEST_IPHONE_DEVICE
```

**Dump symbol sets:**
```bash
ipsw kernel symbolsets kernelcache.release.$LATEST_IPHONE_DEVICE
```
...
```bash
ipsw macho disass kernelcache.release.$LATEST_IPHONE_DEVICE --symbol _kernel_bootstrap
```

**From fileset entry (modern kernelcaches):**
```bash
ipsw macho disass kernelcache.release.$LATEST_IPHONE_DEVICE --fileset-entry "com.apple.kernel" --symbol _kernel_bootstrap
```

**IOKit function:**
```bash
ipsw macho disass kernelcache.release.$LATEST_IPHONE_DEVICE --fileset-entry "com.apple.iokit.IOKit" --symbol _IOLog
```

**KEXT function:**
```bash
ipsw macho disass kernelcache.release.$LATEST_IPHONE_DEVICE --fileset-entry "com.apple.security.sandbox" --symbol _sandbox_check
```
...
```bash
ipsw macho info --objc kernelcache.release.$LATEST_IPHONE_DEVICE | grep "IOUserClient"
```
