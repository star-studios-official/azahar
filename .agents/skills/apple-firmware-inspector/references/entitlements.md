...
```bash
ipsw ent --sqlite entitlements.db --ipsw "$LATEST_IPSW"
```
...
```bash
ipsw ent --sqlite entitlements.db --key com.apple.private.security.sandbox --version "$IOS_VERSION"
```
...
ipsw ent --sqlite "ent_$OLDER_IOS_VERSION.db" --ipsw "$OLDER_IOS_IPSW"
ipsw ent --sqlite "ent_$NEWER_IOS_VERSION.db" --ipsw "$NEWER_IOS_IPSW"

# Query and compare
ipsw ent --sqlite "ent_$OLDER_IOS_VERSION.db" --key "sandbox" > "ent_$OLDER_IOS_VERSION.txt"
ipsw ent --sqlite "ent_$NEWER_IOS_VERSION.db" --key "sandbox" > "ent_$NEWER_IOS_VERSION.txt"
diff "ent_$OLDER_IOS_VERSION.txt" "ent_$NEWER_IOS_VERSION.txt"
```

**Find new private entitlements:**
```bash
ipsw ent --sqlite ent.db --key "com.apple.private" --version "$IOS_VERSION"
```
